...
class Inpainting(H_functions):
    ...
    # puts data from kept indices at beginning, and all missing indices at the end of the vector
    def Vt(self, vec):
        temp = vec.clone().reshape(vec.shape[0], self.channels, -1).permute(0, 2, 1).reshape(vec.shape[0], -1) # not sure what the permuting purpose is to be honest, it might matter if you have more than 1 channel
        out = torch.zeros_like(temp)
...
class SuperInpainting(H_functions):
    def __init__(self, channels, img_dim, missing_indices, ratio, device):
        self.ratio = ratio
        self.channels = channels
        self.img_dim = img_dim
        self._singulars = torch.ones(channels * img_dim**2 - missing_indices.shape[0]).to(device)/ratio
        self.missing_indices = missing_indices
        self.kept_indices = torch.Tensor([i for i in range(channels * img_dim**2) if i not in missing_indices]).to(device).long()
...
# SuperPainting
class SuperPainting(H_functions):
    def __init__(self, channels, img_dim, ratio, device, missing_indices=None): #ratio = 2 or 4
        assert img_dim % ratio == 0
        self.img_dim = img_dim
        self.channels = channels
        self.y_dim = img_dim // ratio
    ...
    def singulars(self):
        temp = self.singulars_small.repeat(self.channels * self.y_dim**2)
        temp[0:temp.shape[0]] = 1
        return temp
